...
Alignment SisdAlignmentEngine::linear(const char* sequence,
    ...
    for (std::uint32_t node_id: rank_to_node_id) {
        ...
        for (std::uint32_t j = 1; j < matrix_width; ++j) {
            ...
            } else if ((type_ == AlignmentType::kNW || type_ == AlignmentType::kROV) &&
                (j == matrix_width - 1 && node->out_edges().empty())) {
                update_max_score(H_row, i, j);

            } else if (type_ == AlignmentType::kOV && node->out_edges().empty()) {
                update_max_score(H_row, i, j);
            } else if (type_ == AlignmentType::kLOV && j == matrix_width - 1) {
                update_max_score(H_row, i, j);
            }
        }
    }
    ...
    std::uint32_t i = std::max(0, max_i);
    std::uint32_t j = std::max(0, max_j);
    ...
    while ((type_ == AlignmentType::kSW && sw_condition()) ||
           ((type_ == AlignmentType::kNW || type_ == AlignmentType::kLOV) && nw_condition()) ||
           ((type_ == AlignmentType::kOV || type_ == AlignmentType::kROV) && ov_condition())) {
